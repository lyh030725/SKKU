...
int Search_prime(Node* T, int k){
    ...
    x->color = 'I';
    BST_INSERT(T, x);
    Node* sus = Successor(x);
    PRINT_BST(T);
    BST_DELETE(&T, x);
    if(sus && sus->key < k){
        printf("key[%d] < %d\n", sus->key ,k);
        printf("Succesor of %d is ", k);
        return Successor(sus)->key;
    }
    else if(sus){
        printf("Succesor of %d is ", k);
        return sus->key;
        ...
        printf("Succesor of %d is not existed\n", k);
        return -1;
    }
